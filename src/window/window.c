#include "window.h"
void windowPollEvents(const State *state) { glfwPollEvents(); }
...
void surfaceCreate(State *state) {
  EXPECT(glfwCreateWindowSurface(state->context.instance, state->window.handle,
...
void surfaceDestroy(State *state) {
  vkDestroySurfaceKHR(state->context.instance, state->window.surface,
                      state->config.allocator);
}
...
VkSurfaceCapabilitiesKHR surfaceCapabilitiesGet(const State *state) {
  ...
  EXPECT(
      vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
          state->context.physicalDevice, state->window.surface, &capabilities),
...
VkSurfaceFormatKHR surfaceFormatsSelect(const State *state) {
  ...
  EXPECT(vkGetPhysicalDeviceSurfaceFormatsKHR(state->context.physicalDevice,
                                              state->window.surface,
                                              &formatCount, NULL),
...
VkPresentModeKHR surfacePresentModesSelect(const State *state) {
  ...
  EXPECT(vkGetPhysicalDeviceSurfacePresentModesKHR(
             state->context.physicalDevice, state->window.surface,
...
void swapchainImagesGet(State *state) {
  EXPECT(vkGetSwapchainImagesKHR(state->context.device,
                                 state->window.swapchain.handle,
                                 &state->window.swapchain.imageCount, NULL),
         "Couldn't get swapchain images count")
  state->window.swapchain.images =
      malloc(state->window.swapchain.imageCount * sizeof(VkImage));
  EXPECT(!state->window.swapchain.images,
         "Couldn't allocate memory for swapchain images")
  EXPECT(vkGetSwapchainImagesKHR(state->context.device,
                                 state->window.swapchain.handle,
                                 &state->window.swapchain.imageCount,
                                 state->window.swapchain.images),
         "Couldn't get swapchain images")
}

void swapchainImageViewsCreate(State *state) {
  state->window.swapchain.imageViews =
      malloc(state->window.swapchain.imageCount * sizeof(VkImageView));
  EXPECT(!state->window.swapchain.imageViews,
  ...
  for (int i = 0; i < state->window.swapchain.imageCount; ++i) {
    EXPECT(vkCreateImageView(
               ...
               state->config.allocator, &state->window.swapchain.imageViews[i]),
...
void swapchainCreate(State *state) {
  ...
  EXPECT(vkCreateSwapchainKHR(
             ...
             &(VkSwapchainCreateInfoKHR){
                 ...
                 .minImageCount = clamp(state->config.swapchainBuffering,
                                        capabilities.minImageCount,
                                        capabilities.maxImageCount
                                            ? capabilities.maxImageCount
                                            : UINT32_MAX),
             },
             state->config.allocator, &state->window.swapchain.handle),
...
void swapchainDestroy(State *state) {
  for (int i = 0; i < state->window.swapchain.imageCount; ++i) {
    vkDestroyImageView(state->context.device,
  ...
  vkDestroySwapchainKHR(state->context.device, state->window.swapchain.handle,
                        state->config.allocator);
}
...
void windowCreate(State *state) {
  ...
  state->window.handle =
      glfwCreateWindow(width, height, state->config.windowTitle, monitor, NULL);
