...
void instanceCreate(State *state) {
  ...
  EXPECT(vkCreateInstance(
             ...
             state->config.allocator, &state->context.instance),
         "Couldn't create instance")
}

void physicalDeviceSelect(State *state) {
  uint32_t count;
  EXPECT(vkEnumeratePhysicalDevices(state->context.instance, &count, NULL),
         "Couldn't enumerate physical devices count")
  EXPECT(count == 0, "Couldn't find a vulkan supported physical device")
  // i should take the count of phisical devices rather than pasing just 1 it
  // dosnt really work like that
  EXPECT(vkEnumeratePhysicalDevices(state->context.instance, &(uint32_t){2},
...
void queueFamilySelect(State *state) {
  ...
  uint32_t count;
  vkGetPhysicalDeviceQueueFamilyProperties(state->context.physicalDevice,
                                           &count, NULL);
  ...
  for (uint32_t queueFamilyIndex = 0; queueFamilyIndex < count;
    ...
    if ((properties.queueFlags & VK_QUEUE_GRAPHICS_BIT) &&
        glfwGetPhysicalDevicePresentationSupport(state->context.instance,
                                                 state->context.physicalDevice,
                                                 queueFamilyIndex)) {
  ...
  EXPECT(state->context.queueFamily == UINT32_MAX,
...
void deviceCreate(State *state) {
  EXPECT(vkCreateDevice(
             ...
             state->config.allocator, &state->context.device),
         "Couldn't create device and queues")
}

void queueGet(State *state) {
  vkGetDeviceQueue(state->context.device, state->context.queueFamily, 0,
                   &state->context.queue);
}
